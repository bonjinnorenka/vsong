...
def groupe_name2men_namev2(groupe_name):#v2のテーブルにアクセス
    ...
    cur.execute("select MN_1,MN_2,MN_3,MN_4,MN_5,MN_6,MN_7,MN_8,MN_9,MN_10,MN_11,MN_12,MN_13,MN_14,MN_15,MN_16,MN_17,MN_18,MN_19,MN_20,MN_21,MN_22,MN_23,MN_24,MN_25,MN_26,MN_27,MN_28,MN_29 from PAIR_LIST_SECOND where groupe_name = '" + groupe_name.replace("'","''") + "'")
...
def make_music_page(music_name):
    ...
    html_body_data_a("""<script>var numbaring=0;var load_max=0;const html=document.querySelector('html');function scroll_ev(){const currentPos=window.pageYOffset;var bottomPoint=document.body.clientHeight-window.innerHeight-600;if(bottomPoint<=currentPos){let request=new XMLHttpRequest();request.open("GET","ajax/tbdata-"+String(numbaring)+".json");numbaring+=1;request.responseType='json';request.send();request.onload=function(){const res=request.response;for(let i=0;i<10;i++){try{let now_tr=document.createElement("tr");now_tr.innerHTML=res[i].mainc;now_tr.className=res[i].forclass;document.getElementById('video_data_tbody').appendChild(now_tr);dt(res[i].fnc[0]);Chart_cleater(res[i].fnc[0],res[i].fnc[1],res[i].fnc[2]);if(i>8){break}}catch{window.removeEventListener('scroll',scroll_ev);if(load_max<2){load_max++;now_ran=Math.floor(Math.random()*100);let request_mr=new XMLHttpRequest();request_mr.open("GET","/ajax/music/mr-"+String(now_ran)+".json");request_mr.responseType='json';request_mr.send();request_mr.onload=function(){const res_mr=request_mr.response;let divm=document.getElementById('music_recommend');document.getElementById('descm').innerHTML='<hr><p class="other_music">他のおすすめの曲</p>';for(let i=0;i<10;i++){sdiv=document.createElement('div');sdiv.innerHTML="<a href='/music/"+res_mr[i][0]+"/'>"+res_mr[i][0]+"<img src='https://i.ytimg.com/vi/"+res_mr[i][1]+"/mqdefault.jpg' alt='"+res_mr[i][0]+"'></a>";divm.appendChild(sdiv)}}; let request_cr=new XMLHttpRequest();request_cr.open("GET","/ajax/ch/cr-"+String(now_ran)+".json");request_cr.responseType='json';request_cr.send();request_cr.onload=function(){const res_cr=request_cr.response;let divc=document.getElementById('ch_recommend');document.getElementById('descc').innerHTML='<hr><p class="other_music">他のおすすめのVtuber</p>';for(let i=0;i<10;i++){divc.innerHTML=divc.innerHTML+"<a href='/ch/"+res_cr[i][0]+"/'><span class='ofoverflow'>"+res_cr[i][0]+"</span><img class='recommend-ch' src='"+res_cr[i][1]+"' alt='"+res_cr[i][0]+"' title='"+res_cr[i][0]+"'></a>"}}}}}}}}window.addEventListener('scroll',scroll_ev);scroll_ev()</script>""")
...
def get_ch_vdata(nickname):
    ...
    menlist = "(" + menlist + ")"
    cur.execute("select video_id from video_id where video_id in (select video_id from video_id where IG = 0 AND CHANNEL_ID = '" + chdata[1] + "' UNION ALL SELECT VIDEO_ID FROM VIDEO_ID WHERE IG = 0 AND GROUPE_NAME in (SELECT GROUPE_NAME FROM PAIR_LIST_SECOND WHERE MN_1 in " + menlist + " OR MN_2 in " + menlist + "OR MN_3 in " + menlist + "OR MN_4 in " + menlist + "OR MN_5 in " + menlist + "OR MN_6 in " + menlist + "OR MN_7 in " + menlist + "OR MN_8 in " + menlist + "OR MN_9 in " + menlist + "OR MN_10 in " + menlist + "OR MN_11 in " + menlist + "OR MN_12 in " + menlist + "OR MN_13 in " + menlist + "OR MN_14 in " + menlist + "OR MN_15 in " + menlist + "OR MN_16 in " + menlist + "OR MN_17 in " + menlist + "OR MN_18 in " + menlist + "OR MN_19 in " + menlist + "OR MN_20 in " + menlist + "OR MN_21 in " + menlist + "OR MN_22 in " + menlist + "OR MN_23 in " + menlist + "OR MN_24 in " + menlist + "OR MN_25 in " + menlist + "OR MN_26 in " + menlist + "OR MN_27 in " + menlist + "OR MN_28 in " + menlist + "OR MN_29 in " + menlist + ") UNION ALL select video_id from video_id where channel_id in (select CH_ID from CH_ID where LINK='" + chdata[1] + "')) and music_name is not null ORDER BY UPLOAD_TIME DESC")
