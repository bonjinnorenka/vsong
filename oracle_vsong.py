...
def todays_hot():
    ...
    rank_list = [[r[0],r[1],math.floor(r[3]*100)] for r in cur.fetchall()]
    ...
    rank_list = [[r[0],r[1],r[3]] for r in cur.fetchall()]
    ...
    rank_list = [[r[0],r[1],r[3]] for r in cur.fetchall()]
    with open(folder_path + siteurl + "/today/index_wholediff.json","w") as f:
        json.dump({"kind":"wholediff","index":rank_list},f)
...
def office_hot():
    ...
    for r in video_office_list:
        ...
        for i in kalist:
            if i[2].date()==nowday_d and i[3] > 0:
                rank_list.append([i[0],i[1],i[3]])
        with open(folder_path + siteurl + f"/today/{dir_name_replace(r)}_wholediff.json","w") as f:
            json.dump({"kind":"wholediff","index":rank_list},f)
