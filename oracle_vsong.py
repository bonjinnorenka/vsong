...
def make_chpage_v2(nick_name,mode=0):
    ...
    for r in range(math.ceil(len(videolist_id)/10)):#メインデータ生成
        ...
        for x in range(10*r,max_len,1):
            ...
            if r==0 and x==0 and mode==0:
                #初回分だけ特殊データ登録
                json_pagedata["first"] = ("<h1><button onclick='allplay()'><img class='control_icon' src='/util/cicle_playbtn.svg'></button>" + nick_name + "</h1><canvas id='sum-yt' class='yt-view-sum inline'></canvas>")
                page_html_data_mdata.append("<h1><button onclick='allplay()'><img class='control_icon' src='/util/cicle_playbtn.svg'></button>" + nick_name + "</h1><canvas id='sum-yt' class='yt-view-sum inline'></canvas>")
