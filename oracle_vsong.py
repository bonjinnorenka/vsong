...
def make_music_page(music_name):
    ...
    for x in range(len(v_data)):
        if x >= 10:#数が増えたとき用10以上になるとAJAX方式に変更
            ...
            can_d,sclist = view_graph(video_id=v_data[x][0],dt=4)
            ...
            can_d,sc = view_graph(video_id=v_data[x][0],dt=3)
        ...
        if v_data[x][2]==1:#歌い手が１人
            if "https://yt" in v_data[x][3][3]:#画像ソースがyoutubeの場合
                html_body_data_a("<a href='/ch/" + v_data[x][3][0] + "' onclick='rec_c()'><img loading='lazy' width='75' height='75' class='v_face' src='" + v_data[x][3][3] + "=s75-c-k-c0x00ffffff-no-rj' alt='" + v_data[x][3][0] + "' title='" + v_data[x][3][0] + "'></a>")
            else:
                html_body_data_a("<a href='/ch/" + v_data[x][3][0] + "' onclick='rec_c()'><img loading='lazy' width='75' height='75' class='v_face' src='" + v_data[x][3][3] + "' alt='" + v_data[x][3][0] + "' title='" + v_data[x][3][0] + "'></a>")
        else:#歌い手が複数人
            for r in range(v_data[x][2]):
                    if "https://yt" in v_data[x][3][r][3]:#画像ソースがyoutubeの場合
                        html_body_data_a("<a href='/ch/" + v_data[x][3][r][0] + "' onclick='rec_c()'><img loading='lazy' width='75' height='75' class='v_face' src='" + v_data[x][3][r][3] + "=s75-c-k-c0x00ffffff-no-rj' alt='" + v_data[x][3][r][0] + "' title='" + v_data[x][3][r][0] + "'></a>")
                    elif "https://pbs.twimg.com" in v_data[x][3][r][3]:#画像ソースがtwitter
                        html_body_data_a("<a href='/ch/" + v_data[x][3][r][0] + "' onclick='rec_c()'><img loading='lazy' width='75' height='75' class='v_face' src='" + str(v_data[x][3][r][3])[:-11] + '200x200' + str(v_data[x][3][r][3])[-4:] + "' alt='" + v_data[x][3][r][0] + "' title='" + v_data[x][3][r][0] + "'></a>")
                    else:
                        html_body_data_a("<a href='/ch/" + v_data[x][3][r][0] + "' onclick='rec_c()'><img loading='lazy' width='75' height='75' class='v_face' src='" + v_data[x][3][r][3] + "' alt='" + v_data[x][3][r][0] + "' title='" + v_data[x][3][r][0] + "'></a>")
        if x >= 10:
    ...
    html_body_data_a("<script>var max_len=" + str(numbering) + """;var numbaring=0;var load_max=0;var su_numb=0;var endcount=0;const html=document.querySelector('html');var urp=new URLSearchParams(new URL(window.location.href));function scroll_do(){if(numbaring<max_len){if(numbaring+2<=max_len){document.head.insertAdjacentHTML('beforeEnd','<link rel="preload" href="ajax/tbdata-'+String(numbaring+1)+'.json" as="fetch">')} let request=new XMLHttpRequest();request.open("GET","ajax/tbdata-"+String(numbaring)+".json");numbaring++;request.responseType='json';request.send();request.onload=function(){const res=request.response;for(let i=0;i<res.ic+1;i++){let now_tr=document.createElement("tr");now_tr.innerHTML=res[i].mainc;now_tr.className=res[i].forclass;document.getElementById("tbd-"+String(res.now_c)).appendChild(now_tr);dt(res[i].fnc[0]);Chart_cleater_v2(res[i].fnc[0],res[i].fnc[1],res[i].fnc[2],res[i].fnc[3],res[i].fnc[4])}}}else{window.removeEventListener('scroll',scroll_ev);if(load_max==0){load_max++;let url_parm=new URL(window.location.href).searchParams;if(url_parm.get('ran')!=null){now_ran=url_parm.get('ran')}else{now_ran=Math.floor(Math.random()*100);urp.delete("tbdid");urp.set("ran",String(now_ran));history.replaceState(null,null,"?"+urp.toString())} let request_mr=new XMLHttpRequest();request_mr.open("GET","/ajax/music/mr-"+String(now_ran)+".json");request_mr.responseType='json';request_mr.send();request_mr.onload=function(){const res_mr=request_mr.response;let divm=document.getElementById('music_recommend');document.getElementById('descm').innerHTML='<hr><p class="other_music">他のおすすめの曲</p>';for(let i=0;i<20;i++){divm.innerHTML=divm.innerHTML+"<a href='/music/"+res_mr[i][0]+"/'>"+res_mr[i][0]+"<img src='https://i.ytimg.com/vi/"+res_mr[i][1]+"/mqdefault.jpg' alt='"+res_mr[i][0]+"'></a>"}};let request_cr=new XMLHttpRequest();request_cr.open("GET","/ajax/ch/cr-"+String(now_ran)+".json");request_cr.responseType='json';request_cr.send();request_cr.onload=function(){const res_cr=request_cr.response;let divc=document.getElementById('ch_recommend');document.getElementById('descc').innerHTML='<hr><p class="other_music">他のおすすめのVtuber</p>';for(let i=0;i<20;i++){divc.innerHTML=divc.innerHTML+"<a href='/ch/"+res_cr[i][0]+"/'><span class='ofoverflow'>"+res_cr[i][0]+"</span><img class='recommend-ch' src='"+res_cr[i][1]+"' alt='"+res_cr[i][0]+"' title='"+res_cr[i][0]+"'></a>"}}}} endcount++} function scroll_ev(){const currentPos=window.pageYOffset;var bottomPoint=document.body.clientHeight-window.innerHeight-600;if(bottomPoint<=currentPos){scroll_do()}} function scroller(){let url_parm=new URL(window.location.href).searchParams;console.log("tp! to "+(url_parm.get('curlen')));window.scroll(0,Number(url_parm.get('curlen')));urp.delete("curlen");history.replaceState(null,null,"?"+urp.toString())} window.addEventListener('scroll',scroll_ev);scroll_do();var set_interval_id=setInterval(fincheck,50);var tryc=0;var nowmax=0;function fincheck(){tryc++;let url_parm=new URL(window.location.href).searchParams;if(tryc>800||url_parm.get('curlen')==null){clearInterval(set_interval_id);delete set_interval_id} if(nowmax===endcount){clearInterval(set_interval_id);if(url_parm.get('curlen')!=null){scroller()}}} function first_do(){let url_parm=new URL(window.location.href).searchParams;if(url_parm.get('curlen')!=null){urp.set('curlen',url_parm.get('curlen'))} if(url_parm.get('ran')!=null){urp.set('ran',url_parm.get('ran'))} if(url_parm.get('ran')!=null){k_dols=[];nowmax=max_len;for(let step=0;step<max_len;step++){scroll_do()}} if(url_parm.get('tbdid')!=null){nowmax=Number(url_parm.get('tbdid'))+1;for(let step=0;step<1+Number(url_parm.get('tbdid'));step++){scroll_do()}}} function rec_c(){urp.set('curlen',String(window.pageYOffset));history.replaceState(null,null,"?"+urp.toString())} first_do()</script>""")
...
def make_channel_page(nick_name):#チャンネルのページを作成
    ...
    for x in range(len(v_data)):
        ...
        if x >= 10:#数が増えたとき用10以上になるとAJAX方式に変更
            ...
            html_body_data_a = tmp_datalist.append
            can_d,sclist = view_graph(video_id=v_data[x][0],dt=4)
            html_body_data_a("<td><lite-youtube videoid='" + v_data[x][0] + "' playlabel='Play'></lite-youtube></td><td class='yt-meta' id='" + v_data[x][0] + "_td'><details id='" + v_data[x][0] + "_dt' class='mdata-dt'><summary class='music_title'>" + v_data[x][1][0] + "</summary>" + can_d)
        else:
            can_d,sc = view_graph(video_id=v_data[x][0],dt=3)
            html_body_data_a("<tr class='" + " ".join(men_of_list) + "'><td><lite-youtube videoid='" + v_data[x][0] + "' playlabel='Play'></lite-youtube></td><td class='yt-meta' id='" + v_data[x][0] + "_td'><details id='" + v_data[x][0] + "_dt' class='mdata-dt'><summary class='music_title'>" + v_data[x][1][0] + "</summary>" + can_d)
        html_body_data_a("<a href='https://vsong.fans/video_data/" + v_data[x][0] + ".html'>詳細データ</a><div class='vtuber_sing'>")
        if v_data[x][2]==1:#歌い手が１人
            if "https://yt" in v_data[x][3][3]:#画像ソースがyoutubeの場合
                html_body_data_a("<a href='/ch/" + v_data[x][3][0] + "' onclick='rec_c()'><img loading='lazy' width='75' height='75' class='v_face' src='" + v_data[x][3][3] + "=s75-c-k-c0x00ffffff-no-rj' alt='" + v_data[x][3][0] + "' title='" + v_data[x][3][0] + "'></a>")
        ...
        if x in range(10,1000,10):
            html_body_data.append("</tbody><tbody id='tbd-" + str(numbering_b) + "'>")
            numbering_b += 1
        if x>=10 and len(v_data)-1==x or x+1 in range(20,1000,10):#10以上かつ最後か20以上の10のときにAJAX用ファイルを生成
            if os.path.isdir(n_html_path + "ajax/")==False:#フォルダがなければ生成
                os.mkdir(n_html_path + "ajax/")
            overflow_ajax["now_c"] = numbering
            overflow_ajax["rec"] = ic
    ...
    html_body_data_a = html_body_data.append#変更している場合があるのでこのタイミングで元に戻す
    html_body_data_a("""</tbody></table></div><div id="descm"></div><div id="music_recommend"></div><div id="descc"></div><div id="ch_recommend"></div></main>""")
    html_body_data_a("<script>var max_len=" + str(numbering) + """;var numbaring=0;var load_max=0;var su_numb=0;var endcount=0;var urp=new URLSearchParams(new URL(window.location.href));const html=document.querySelector("html");async function scroll_do(str){if(numbaring<max_len){document.head.insertAdjacentHTML('beforeEnd','<link rel="preload" href="ajax/tbdata-'+String(numbaring+1)+'.json" as="fetch">');let request=new XMLHttpRequest();request.open("GET","ajax/tbdata-"+String(numbaring)+".json");numbaring++;request.responseType="json";request.send();request.onload=function(){const res=request.response;let url_parm=new URL(window.location.href).searchParams;if(url_parm.get('ran')==null&&Number(url_parm.get('tbdid')<=su_numb)){urp.set("tbdid",String(su_numb));history.replaceState(null,null,"?"+urp.toString())} su_numb++;for(let i=0;i<10;i++){let now_tr=document.createElement("tr");now_tr.innerHTML=res[i].mainc;now_tr.className=res[i].forclass;document.getElementById("tbd-"+String(res.now_c)).appendChild(now_tr);dt(res[i].fnc[0]);Chart_cleater_v2(res[i].fnc[0],res[i].fnc[1],res[i].fnc[2],res[i].fnc[3],res[i].fnc[4])} scroll_ev()}}else if(load_max==0){window.removeEventListener("scroll",scroll_ev);load_max++;let url_parm=new URL(window.location.href).searchParams;if(url_parm.get('ran')!=null){now_ran=url_parm.get('ran')}else{now_ran=Math.floor(Math.random()*100);urp.delete("tbdid");urp.set("ran",String(now_ran));history.replaceState(null,null,"?"+urp.toString())} let request_mr=new XMLHttpRequest();request_mr.open("GET","/ajax/music/mr-"+String(now_ran)+".json");request_mr.responseType="json";request_mr.send();request_mr.onload=function(){const res_mr=request_mr.response;let divm=document.getElementById("music_recommend");document.getElementById("descm").innerHTML='<hr><p class="other_music">他のおすすめの曲</p>';for(let i=0;i<20;i++){divm.innerHTML=divm.innerHTML+"<a href='/music/"+res_mr[i][0]+"/' onclick='rec_c()'>"+res_mr[i][0]+"<img src='https://i.ytimg.com/vi/"+res_mr[i][1]+"/mqdefault.jpg' alt='"+res_mr[i][0]+"'></a>"}};let request_cr=new XMLHttpRequest();request_cr.open("GET","/ajax/ch/cr-"+String(now_ran)+".json");request_cr.responseType="json";request_cr.send();request_cr.onload=function(){const res_cr=request_cr.response;let divc=document.getElementById("ch_recommend");document.getElementById("descc").innerHTML='<hr><p class="other_music">他のおすすめのVtuber</p>';for(let i=0;i<20;i++){divc.innerHTML=divc.innerHTML+"<a href='/ch/"+res_cr[i][0]+"/' onclick='rec_c()'><span class='ofoverflow'>"+res_cr[i][0]+"</span><img class='recommend-ch' src='"+res_cr[i][1]+"' alt='"+res_cr[i][0]+"' title='"+res_cr[i][0]+"'></a>"}}} endcount++} function scroll_ev(){const currentPos=window.pageYOffset;var bottomPoint=document.body.clientHeight-window.innerHeight-600;if(bottomPoint<=currentPos){scroll_do()}} function scroller(){let url_parm=new URL(window.location.href).searchParams;console.log("tp! to "+(url_parm.get('curlen')));window.scroll(0,Number(url_parm.get('curlen')));urp.delete("curlen");history.replaceState(null,null,"?"+urp.toString())} window.addEventListener("scroll",scroll_ev);var set_interval_id=setInterval(fincheck,50);var tryc=0;var nowmax=0;function fincheck(){tryc++;let url_parm=new URL(window.location.href).searchParams;if(tryc>800||url_parm.get('curlen')==null){clearInterval(set_interval_id);delete set_interval_id} if(nowmax===endcount){clearInterval(set_interval_id);if(url_parm.get('curlen')!=null){scroller()}}} function first_do(){let url_parm=new URL(window.location.href).searchParams;if(url_parm.get('curlen')!=null){urp.set('curlen',url_parm.get('curlen'))} if(url_parm.get('ran')!=null){urp.set('ran',url_parm.get('ran'))} if(url_parm.get('ran')!=null){k_dols=[];nowmax=max_len;for(let step=0;step<max_len;step++){scroll_do()}} if(url_parm.get('tbdid')!=null){nowmax=Number(url_parm.get('tbdid'))+1;for(let step=0;step<1+Number(url_parm.get('tbdid'));step++){scroll_do()}}} function rec_c(){urp.set('curlen',String(window.pageYOffset));history.replaceState(null,null,"?"+urp.toString())} first_do();scroll_do()</script>""")
